 ...
 */
@Component
public class WechatMatPipeline implements Pipeline {
    @Autowired
    ...
    public void process(ResultItems resultItems, Task task) {
        ...
        if (commentVoList != null && commentVoList.size() > 0) {
            try {
                wechatExtopiaMapper.matAdd(commentVoList);
            } catch (Exception e) {
                for (CommentVo commentVo:commentVoList)
                {
                    try {
                        wechatExtopiaMapper.matSingleAdd(commentVo);
                    }catch (Exception e1)
                    {
                        logger.error("插入数据库异常 WechatMatPipeline："+ JSON.toJSONString(commentVo)+"异常信息："+e1.fillInStackTrace());
                    }
