 ...
 */
@Component("BExtopiaPipeline")
public class BExtopiaPipeline implements Pipeline {
    @Autowired
    private bMapper bMapper;
    private Logger logger = LoggerFactory.getLogger(BExtopiaPipeline.class);
    ...
    public void process(ResultItems resultItems, Task task) {
        ...
        if (commentVo != null) {
            String time = PropertiesUtils.readData("spiderLastTime.properties", "extopiaB");
    ...
    private void save(List<CommentDto> commentVoDtos) {
        ...
        if (all > 0) {
            if (all <= limit) {
                try {
                    bMapper.extopiaAdd(commentVoDtos);
                } catch (Exception e) {
                    List<CommentDto> commentDtos = commentVoDtos;
                    for (CommentDto dto : commentDtos) {
                        try {
                            bMapper.extopiaSingleAdd(dto);
                        } catch (Exception e1) {
                ...
                while (currentSize < all) {
                    if (allPage == (cunrrentPage + 1)) {
                        try {
                            bMapper.extopiaAdd(commentVoDtos.subList(cunrrentPage * limit, all));
                        } catch (Exception e) {
                        ...
                        try {
                            bMapper.extopiaAdd(commentVoDtos.subList(cunrrentPage * limit, (cunrrentPage + 1) * limit));
                        } catch (Exception e) {
