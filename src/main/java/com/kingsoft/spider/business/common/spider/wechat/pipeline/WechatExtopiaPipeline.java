 ...
 */
@Component
public class WechatExtopiaPipeline implements Pipeline {
    @Autowired
    ...
    public void process(ResultItems resultItems, Task task) {
        ...
        {
           try {
               wechatExtopiaMapper.extopiaAdd(commentVoList);
           }catch (Exception e)
           {
               for (CommentVo commentVo:commentVoList)
               {
                   try {
                       wechatExtopiaMapper.extopiaSingleAdd(commentVo);
                   }catch (Exception e1)
                   {
                       logger.error("插入数据库异常 WechatExtopiaPipeline："+ JSON.toJSONString(commentVo)+"异常信息："+e1.fillInStackTrace());
                   }
